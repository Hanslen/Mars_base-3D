#include "Camera.h"
#include "VectorMath.h"
#include "Scene.h"
#include <iostream>
using namespace std;
Camera::Camera() : wKey(0), sKey(0), aKey(0), dKey(0), rKey(0), currentButton(0), mouseX(0), mouseY(0),driver(0),driverRotate(100.f),changeDegree(0.f)
{
    driverEyePosition[0] = -50.f;
    driverEyePosition[1] = -100+50.f;
    driverEyePosition[2] = -200.f;
	Reset();
}

void Camera::Reset(){
	// set the camera position to start at (0,0,0)
    if(driver == 1){
        driverEyePosition[0] = eyePosition[0];
        driverEyePosition[1] = eyePosition[1];
        driverEyePosition[2] = eyePosition[2];
    }

	eyePosition[0] = 0.0f;
	eyePosition[1] = 0.0f;
	eyePosition[2] = 0.5f * static_cast<float>(Scene::GetWindowHeight()) / static_cast<float>(tan(M_PI / 6.0));//0.0f;

	// set the view direction vector of the camera to be (0,0,-1)
	vd[0] = 0.0f;
	vd[1] = 0.0f;
	vd[2] = -1.0f;

	// set the planar forward direction vector of the camera to be (0,0,-1)
	forward[0] = 0.0f;
	forward[1] = 0.0f;
	forward[2] = -1.0f;

	// set the right vector to point along the x axis
	right[0] = 1.0f;
	right[1] = 0.0f;
	right[2] = 0.0f;

	// set the up vector of the camera to be up the y axis
	up[0] = 0.0f;
	up[1] = 1.0f;
	up[2] = 0.0f;
    driver = 0;
}

void Camera::SetViewport()
{
	glViewport(static_cast<GLint>(0), static_cast<GLint>(0), static_cast<GLsizei>(Scene::GetWindowWidth()), static_cast<GLsizei>(Scene::GetWindowHeight()));
}

void Camera::SetupCamera()
{
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	gluLookAt(eyePosition[0], eyePosition[1], eyePosition[2],
			  eyePosition[0] + vd[0], eyePosition[1] + vd[1], eyePosition[2] + vd[2],
			  up[0], up[1], up[2]);

}

void Camera::Update(const double& deltaTime)
{
//    cout<<driver<<endl;
	float speed = 1.0f;
    int circle = int(driverRotate/(3.1415926*2));
    float modRotate = driverRotate-3.1415926*2*circle;
	if (aKey)
		sub(eyePosition, right, speed);

	if (dKey)
		add(eyePosition, right, speed);

	if (wKey)
		add(eyePosition, forward, speed);

	if (sKey)
		sub(eyePosition, forward, speed);
//    cout<<rKey<<driver<<endl;
    if(rKey && driver != 100){
        setDriver(0);
        if(driverRotate == 100.f){
            vd[0] = -1.0f;
            vd[1] = 0.0f;
            vd[2] = 0.0f;
        }
        if(modRotate<1.5707963){//pi/2
            vd[0] = (-1)/tan(driverRotate);
            vd[2] = 1;
        }
        else if(modRotate<3.1415926){
            vd[0] = -1/tan(driverRotate);
            vd[2] = 1;
        }
        else if(modRotate<4.7123889){
            vd[0] = 1/tan(driverRotate);
            vd[2] = -1;
        }
        else if(modRotate<6.2831852){
            vd[0] = 1/tan(driverRotate);
            vd[2] = -1;
        }
        
        right[0] = 0.0f;
        right[1] = 0.0f;
        right[2] = -1.0f;
        forward[0] = -1.0f;
        forward[1] = 0.0f;
        forward[2] = 0.0f;
        eyePosition[0] = driverEyePosition[0];
        eyePosition[1] = driverEyePosition[1];
        eyePosition[2] = driverEyePosition[2];
        rKey = 0;
        
    }
    if(driver == 1){
        //rotate eye
        if(tan(driverRotate)>0 && vd[0] < 0){
            vd[0] = (-1)/tan(driverRotate);
            vd[2] = 1;
        }
        else if(tan(driverRotate)<0 && vd[2] >0){
            vd[0] = -1/tan(driverRotate);
            vd[2] = 1;
        }
        else if(tan(driverRotate)>0 && vd[0] >0){
            vd[0] = 1/tan(driverRotate);
            vd[2] = -1;
        }
        else if(tan(driverRotate)<0 && vd[2] <0){
            vd[0] = 1/tan(driverRotate);
            vd[2] = -1;
        }
        norm(vd);
        cross(vd, up, right);
        norm(right);
        forward[0] = vd[0];
        forward[2] = vd[2];
        norm(forward);

        add(eyePosition, forward, 20*sin(changeDegree)*10);
        add(eyePosition, right, 20*10*(1-cos(changeDegree)));
        
//        eyePosition[0] += driverEyePosition[0];
//        eyePosition[1] += driverEyePosition[1];
//        eyePosition[2] += driverEyePosition[2];
    }

	SetupCamera();
}
void Camera::setDriverRotate(float degree, float degree2){
    driverRotate = (degree-90)/180*3.1415926;
    changeDegree = degree2/180*3.1415926;
}
void Camera::SetDriverEyePosition(float x, float y, float z){
    driverEyePosition[0] = x;
    driverEyePosition[1] = y;
    driverEyePosition[2] = z;
}
int Camera::getDriver(){
    return driver;
}
void Camera::setDriver(int stop){
    if(stop == 100){
        driver = 100;
    }
    else{
        if(driver == 1){
            driver = 0;
        }
        else{
            driver = 1;
        }
    }
}

void Camera::GetEyePosition(float &x, float &y, float &z) const
{
	x = eyePosition[0];
	y = eyePosition[1];
	z = eyePosition[2];
}

void Camera::GetViewDirection(float &x, float &y, float &z) const
{
	x = vd[0];
	y = vd[1];
	z = vd[2];
}

void Camera::GetForwardVector(float &x, float &y, float &z) const
{
	x = forward[0];
	y = forward[1];
	z = forward[2];
}

void Camera::GetRightVector(float &x, float &y, float &z) const
{
	x = right[0];
	y = right[1];
	z = right[2];
}

void Camera::GetUpVector(float &x, float &y, float &z) const
{
	x = up[0];
	y = up[1];
	z = up[2];
}

void Camera::HandleKey(unsigned char key, int state, int x, int y)
{
    if(driver != 1){
        switch (key)
        {
            case 'A':
            case 'a':
                aKey = state;
                break;
            case 'D':
            case 'd':
                dKey = state;
                break;
            case 'W':
            case 'w':
                wKey = state;
                break;
            case 'S':
            case 's':
                sKey = state;
                break;
            case 'R':
            case 'r':
                rKey = state;
                break;
            case ' ':
                Reset();
            default:
                break;
        }
    }
    switch (key) {
        case ' ':
            Reset();
            break;
            
        default:
            break;
    }
}

void Camera::HandleMouse(int button, int state, int x, int y)
{
//    cout<<button<<" "<<state<<" "<<x<<" "<<y<<endl;
    if(driver == 0){
        currentButton = button;
        mouseX = x;
        mouseY = y;
    }
}

void Camera::HandleMouseDrag(int x, int y)
{
    if(driver == 0){
	float rx, ry;
	float sensitivity = 0.01f; // speed of the camera moving
    
	// work out the difference between where the mouse was last used (mouseX, mouseY) to
	// position the view direction and the current position (x, y) the mouse is in
	rx = static_cast<float>(x - mouseX);
	ry = static_cast<float>(y - mouseY);
    
	// switch on which button was pressed and only do the update if the left button was pressed
	switch (currentButton)
	{
		case GLUT_LEFT_BUTTON:

			// add on the amount of change in to the left and right view direction of the camera
			if (rx > 0)
				add(vd, right, rx*sensitivity);
			else
				sub(vd, right, rx*-sensitivity);
			// add on the amount of change in to the up and down view direction of the camera
			if (ry > 0)
				sub(vd, up, ry*sensitivity);
			else
				add(vd, up, ry*-sensitivity);

			// normalise the view direction so it is length 1
			norm(vd);

			// use the view direction crossed with the up vector to obtain the corrected right vector
			cross(vd, up, right);

			// normalise the right vector
			norm(right);

			/* As we want out camera to stay on the same plane at the same height (i.e. not move up and down the y axis) update a forward direction vector which can be used to move the camera. This forward vector moves the camera in the same direction as the view direction except it will not contain any y component so it cannot move off of its original height. This was we are free to look up and down however moving forward and back will not move us off of the camera plane. */

			forward[0] = vd[0];
			forward[2] = vd[2];
			norm(forward);
			break;
		case GLUT_RIGHT_BUTTON:
			break;
		case GLUT_MIDDLE_BUTTON:
			break;
		default:
			break;
	}

	mouseX = x;
	mouseY = y;
    }
}
